        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            try:
                ...
                m.stride = torch.tensor([s / x.shape[-2] for x in forward(
                        torch.zeros(2, 6, s, s).to(torch.device('cuda')))]) 
            except RuntimeError:
                try:
                    self.model.to(torch.device('cuda'))
                    m.stride = torch.tensor([s / x.shape[-2] for x in forward(
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [c1, c2, *args[1:]]
            #print(len(args))
            if m in {BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3,C2f_PKIModule,C2f_FEM,C2f_PPA,C2f_RG,C2f_Faster,A2C2f,C3k2}:
...
#            print("ch[f]", f, ch[f[0]])
            ...
            args = [ch[f], *args]
        elif m is returnlowir:
            c1,c2=ch[f],args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1,c2]
        elif m is returnlowrgb:
            c1,c2=ch[f],args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1,c2]
        elif m is returnfeature:
            c1, c2 = ch[f[0]], args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1,c2]
            ...
            # print("Add2 arg", args[0])
            args = [c1, c2]
        elif m is fusehdwithoutlow:
            c1, c2 = ch[f[0]], args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
            # print("Add2 arg", args[0])
            args = [c1, c2]
        elif m is fusehdwithoutlh:
            c1, c2 = ch[f[0]], args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
            # print("Add2 arg", args[0])
            args = [c1, c2]
        elif m is fusehdwithouthl:
            c1, c2 = ch[f[0]], args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
            # print("Add2 arg", args[0])
            args = [c1, c2]
        elif m is fusehdwithouthh:
            c1, c2 = ch[f[0]], args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
            # print("Add2 arg", args[0])
            args = [c1, c2]
        elif m is fusehdxiaorong:
            c1, c2 = ch[f[0]], args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
            # print("Add2 arg", args[0])
            args = [c1, c2]
        elif m is fusehdxiaorong2:
            c1, c2 = ch[f[0]], args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
            # print("Add2 arg", args[0])
            args = [c1, c2]
        elif m is fusehdxiaorong3:
            c1, c2 = ch[f[0]], args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
            # print("Add2 arg", args[0])
            args = [c1, c2]
        elif m is fusehdxiaorong4:
            c1, c2 = ch[f[0]], args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
            # print("Add2 arg", args[0])
            args = [c1, c2]
        elif m is fusehdquanzhong:
            c1, c2 = ch[f[0]], args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
            # print("Add2 arg", args[0])
            args = [c1, c2]
        elif m is getreturnir:
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
            # print("Add2 arg", args[0])
            args = [c1, c2]
        elif m is getreturnfuse:
            c1, c2 = 2*ch[f], args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8)
            #print("Add2 arg", args[0])
            args = [c1, c2]
        elif m is getreturnrgb:
           c1, c2 = ch[f], args[0]
           if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
            # print("Add2 arg", args[0])
           args = [c1, c2]
        elif m is getreturnfeature:
           c1, c2 = ch[f], args[0]
           if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8)
            # print("Add2 arg", args[0])
           args = [c1, c2]
        elif m is getreturnfeature2:
           c1, c2 = ch[f[0]], args[0]
           if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
            # print("Add2 arg", args[0])
           args = [c1, c2]
        elif m is getreturnfhigh:
            c1=256
            c2=args[0]
            if c2 != nc:  # if not output
               c2 = make_divisible(min(c2, max_channels) * width, 8) 
