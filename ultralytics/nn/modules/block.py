...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad, chnnel_swapping_Vmamba
...
from .util import resize_pos_embed, DropPath
from .wavelet import create_wavelet_filter, wavelet_transform, inverse_wavelet_transform
#T_MAX = 8192000000000 # increase this if your ctx_len is long [NOTE: TAKES LOTS OF VRAM!]
...
class returnlowrgb(nn.Module):
    ...
    def forward(self,x):
        ...
        xrgblow=x[1]
        return x[1]
class returnfeature(nn.Module):
    def __init__(self,c1,c2):
        super().__init__()
        self.wt_filterir, self.iwt_filterir = create_wavelet_filter("haar", c1, c1, torch.float)
        #self.wt_filterir = nn.Parameter(self.wt_filterir, requires_grad=False)
        self.iwt_filterir = nn.Parameter(self.iwt_filterir, requires_grad=False)
        #self.wt_functionir = partial(wavelet_transform, filters = self.wt_filterir)
        self.iwt_functionir = partial(inverse_wavelet_transform, filters = self.iwt_filterir)
        #self.wt_filterrgb, self.iwt_filterrgb = create_wavelet_filter("haar", c1, c1, torch.float)
        # self.c1=c1
        #self.wt_filterrgb = nn.Parameter(self.wt_filterrgb, requires_grad=False)
        #self.iwt_filterrgb = nn.Parameter(self.iwt_filterrgb, requires_grad=False)
        #self.wt_functionrgb = partial(wavelet_transform, filters = self.wt_filterrgb)
        #self.iwt_functionrgb = partial(inverse_wavelet_transform, filters = self.iwt_filterrgb)
    def forward(self,x):
        xfuselow=x[0]
        xrgbhigh=x[1][3]
        xirhigh=x[1][2]
        xfusehigh=xrgbhigh+xirhigh
        xfuselow=torch.unsqueeze(xfuselow,2)
        xfuse=torch.cat([xfuselow,xfusehigh],dim=2)
        xreturn = self.iwt_functionir(xfuse)
        return xreturn
###########################above is xiaoboronghenewchangehead##############################################
...
class ShuffleNetV2(nn.Module):
    def __init__(self, inp, oup, stride):  # ch_in, ch_out, stride
        ...
        branch_features = oup // 2 # 输出的一�?        assert (self.stride != 1) or (inp == branch_features << 1)
...
class ResNetBlock2(nn.Module):
    ...
    def __init__(self, c1, c2, s=1, e=4):
        ...
        c3 = e * c2
        self.cv1 = Conv(c1, c2, k=1, s=1, act=True)
        self.cv2 = Conv(c2, c2, k=1, s=1, act=True)
        self.cv3 = Conv(c2, c3, k=1, act=False)
        self.shortcut = nn.Sequential(Conv(c1, c3, k=1, s=1, act=False)) if s != 1 or c1 != c3 else nn.Identity()
...
class ResNetLayer2(nn.Module):
    ...
    def __init__(self, c1, c2, s=1, is_first=False, n=1, e=4):
        ...
        if self.is_first:
            ...
            blocks = [ResNetBlock2(c1, c2, 1, e=e)]
...
class SqueezeExcite(nn.Module):
    def __init__(self, in_chs, se_ratio=0.25, reduced_base_chs=None,
        ...
        self.gate_fn = gate_fn   # 激活函�?        reduced_chs = _make_divisible((reduced_base_chs or in_chs) * se_ratio, divisor)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
...
class Fusion_module(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        global_w = self.global_att(agg_input) ## 全局注意�?即channel attention
        w = self.sigmoid(local_w * global_w) ## 计算特征x1的权�?        xo = w * x1 + (1 - w) * x2 ## fusion results ## 特征聚合
        return xo
