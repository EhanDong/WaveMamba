...
class Annotator:
    ...
    def masks(self, masks, colors, im_gpu, alpha=0.5, retina_masks=False):
        ...
        im_gpu = im_gpu[:,:,0:3] * inv_alpha_masks[-1] + mcs#xiugai
        im_mask = im_gpu * 255
        im_mask_np = im_mask.byte().cpu().numpy() 
        self.im=self.im[:,:,0:3]#0:3 IR xiugai
    ...
    if isinstance(x,tuple):
       if len(x)==20:
         for k in range(0,len(x)):
           ...
           if k==3:
              continue'''
           _, channels, height, width = x[k].shape  # batch, channels, height, width
           if height > 1 and width > 1:
              f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_features{k}.png"  # filename

              blocks = torch.chunk(x[k][0].cpu(), channels, dim=0)
              #print(blocks.shape)  # select batch index 0, block by channels
              ...
              for i in range(n):
                    ...
                    plt.axis("off")
                    f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_features{k}_{i}.png"
                    plt.savefig(f, dpi=300, bbox_inches="tight")
                    plt.close()
                    LOGGER.info(f"Saving {f}... ({n}/{channels}_{k}_{i})")
          ...
          for k in range(0,len(x)):
           ...
           if height > 1 and width > 1:
              ...
              for i in range(n):
                    ...
                    plt.savefig(f, dpi=300, bbox_inches="tight")
                    plt.close()
                    LOGGER.info(f"Saving {f}... ({n}/{channels}{i})")
              #np.save(str(f.with_suffix(".npy")), x[0].cpu().numpy()) 
    else:  
         ...
         if height > 1 and width > 1:
              ...
              for i in range(n):
                    #print(blocks[i].squeeze().shape)
                    #print(blocks[i])
                    plt.imshow(blocks[i].squeeze())  # cmap='gray'
                    plt.axis("off")
                    f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_features{i}.png"
                    plt.savefig(f, dpi=300, bbox_inches="tight")
                    plt.close()
                    LOGGER.info(f"Saving {f}... ({n}/{channels}{i})")
              #LOGGER.info(f"Saving {f}... ({n}/{channels})")
